...
import { ShieldPlus, Menu, LogOut, Calendar, PlusCircle, BarChart3, Users, Building2, Activity } from 'lucide-react';
    ...
    <div className="bg-background min-h-screen text-on-background font-body-md">
      <header className="fixed top-0 w-full z-50 h-16 bg-white/95 backdrop-blur-md border-b border-surface-variant flex justify-between items-center px-4 lg:px-6">
        <div className="flex items-center gap-3">
          <button onClick={vm.toggleMobileMenu} className="lg:hidden p-2"><Menu /></button>
          <div className="text-lg font-bold text-primary flex items-center gap-2"><ShieldPlus /> RedNorte</div>
        </div>
        <div className="flex items-center gap-4">
          <div className="hidden md:flex flex-col items-end mr-2"><span className="font-bold">{vm.userName}</span><span className="text-xs">{vm.userRole}</span></div>
          <button onClick={vm.handleCerrarSesion} className="text-error p-2"><LogOut /></button>
        </div>
      </header>
      <aside className={`fixed left-0 top-16 h-[calc(100vh-4rem)] w-64 bg-surface-container-lowest border-r border-surface-variant z-40 transition-transform ${vm.isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <nav className="p-4 flex flex-col gap-2">
          {menuItems.map(item => (
            <Link key={item.path} to={item.path} onClick={vm.closeMobileMenu} className={`flex items-center gap-3 px-4 py-3 rounded-full text-sm ${location.pathname.startsWith(item.path) ? 'bg-secondary-container font-bold' : 'hover:bg-surface-container-low'}`}>
              <item.icon className="w-5 h-5" /> {item.name}
            </Link>
          ))}
        </nav>
      </aside>
      <main className="pt-16 lg:ml-64 min-h-screen"><div className="p-6 md:p-8 max-w-7xl mx-auto"><Outlet /></div></main>
    </div>
  );
}