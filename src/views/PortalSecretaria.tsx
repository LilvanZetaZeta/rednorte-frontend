...
export default function PortalSecretaria() {
  ...
  const onCrearCita = async (e: React.FormEvent) => {
    const res = await vm.handleCrearCitaSubmit(e);
    if(res) showMsg(res.success ? res.message! : res.error!, res.success ? 'success' : 'error');
  };
  ...
  return (
    <div className="animate-in fade-in duration-700 space-y-8 pb-10 relative">
      ...
      <Modal
        ...
        maxWidth="md"
        footer={<div className="flex gap-3"><Button variant="outline" type="button" onClick={() => vm.setShowModalNuevaCita(false)} className="flex-1">Cancelar</Button><Button type="submit" form="form-nueva-cita" className="flex-1">Confirmar Cita</Button></div>}
      >
        <form id="form-nueva-cita" onSubmit={onCrearCita} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-on-surface ml-1 mb-1">RUT Paciente *</label>
            <div className="flex gap-2">
              <Input required type="text" value={vm.formData.rut} onChange={e => vm.setFormData({...vm.formData, rut: vm.formatearRut(e.target.value)})} placeholder="12.345.678-9" disabled={vm.rutVerificado} className="flex-1 font-mono" />
              <Button type="button" variant="secondary" onClick={vm.handleValidarRutPaciente} className="!py-2">Validar</Button>
            </div>
            {vm.validacionRutError && <p className="mt-2 text-sm text-red-600">{vm.validacionRutError}</p>}
            {vm.rutVerificado && <p className="mt-2 text-sm text-green-700">{vm.pacienteExiste ? 'Paciente registrado. Complete con los datos de la cita.' : 'Nuevo paciente. Ingrese nombre y correo para registrarlo.'}</p>}
          </div>

          {vm.rutVerificado && vm.pacienteExiste === false && (
            <div className="p-4 bg-blue-50 rounded-lg border border-blue-100 space-y-3">
              <Input label="Nombre Completo *" required type="text" value={vm.formData.nombre} onChange={e => vm.setFormData({...vm.formData, nombre: e.target.value})} />
              <Input label="Correo Electrónico *" required type="email" value={vm.formData.correo} onChange={e => vm.setFormData({...vm.formData, correo: e.target.value})} />
            </div>
          )}
          ...
          {vm.rutVerificado && (
            <>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-on-surface ml-1 mb-1">Filtrar por Especialidad</label>
                  <select value={vm.filtroEspecialidadId} onChange={e => vm.setFiltroEspecialidadId(e.target.value ? Number(e.target.value) : '')} className="w-full py-3.5 px-5 rounded-2xl bg-surface-container-high border-none outline-none focus:ring-2 focus:ring-primary transition-all text-sm">
                    <option value="">Todas las especialidades</option>
                    {vm.especialidadesDisponibles.map((esp) => <option key={esp.id} value={esp.id}>{esp.nombre}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-on-surface ml-1 mb-1">Seleccionar Médico *</label>
                  <select required value={vm.formData.medicoId} onChange={e => vm.setFormData({...vm.formData, medicoId: e.target.value})} className="w-full py-3.5 px-5 rounded-2xl bg-surface-container-high border-none outline-none focus:ring-2 focus:ring-primary transition-all text-sm">
                    <option value="">Seleccione un médico de su centro...</option>
                    {vm.doctoresFiltrados.map(m => <option key={m.id} value={m.id}>{m.nombreCompleto} - {m.especialidades?.[0]?.nombre || 'General'}</option>)}
                  </select>
                </div>
                <Input label="Fecha de la Cita *" required type="date" min={new Date().toISOString().slice(0, 10)} value={vm.fechaSeleccionada} onChange={e => { vm.setFechaSeleccionada(e.target.value); vm.setFormData({...vm.formData, fechaHora: ''}); }} />
                {vm.fechaSeleccionada && vm.formData.medicoId && (
                  <div>
                    <label className="block text-sm font-medium text-on-surface ml-1 mb-1">Horas Disponibles *</label>
                    {vm.horasDisponibles.length > 0 ? (
                      <div className="grid grid-cols-2 gap-2">
                        {vm.horasDisponibles.map((hora) => (
                          <button key={hora} type="button" onClick={() => vm.setFormData({...vm.formData, fechaHora: hora})} className={`px-3 py-2 border rounded-lg text-left transition-colors ${vm.formData.fechaHora === hora ? 'bg-primary text-white border-primary' : 'bg-surface-container-lowest text-on-surface hover:bg-surface-container-high'}`}>
                            {new Date(hora).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </button>
                        ))}
                      </div>
                    ) : <p className="text-sm text-slate-500">No hay horas disponibles para esta fecha.</p>}
                  </div>
                )}
              </div>
            </>
      ...
      >
        <form id="form-bloqueo-agenda" onSubmit={onBloqueo} className="space-y-4">
          ...
          <Input label="ID Médico Ausente *" required type="number" value={vm.bloqueoData.medicoId} onChange={e => vm.setBloqueoData({...vm.bloqueoData, medicoId: e.target.value})} placeholder="Ej: 42" />
