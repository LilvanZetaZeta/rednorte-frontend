...
export const usuariosApi = apiGateway.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getCandidatosStaff: builder.query<IUsuario[], void>({
      query: () => '/portal/usuarios/candidatos-staff',
      providesTags: ['Usuarios'],
    }),
    ...
    getUsuarioPorRut: builder.query<IUsuario, string>({
      query: (rut) => `/portal/usuarios/rut/${encodeURIComponent(rut)}`,
      providesTags: ['Usuarios'],
    }),
...
export const {
  ...
  useGetUsuarioPorRutQuery,
  useLazyGetUsuarioPorRutQuery,
